...
class w_1:
    ...
    def http_request(self,GP):
        if GP=='get':
            a=requests.get(self.url,self.main)
        elif GP=='post':
            a=requests.post(url,data=self.main)
        return  ('响应报文:{}响应头:{}状态码:{}'.format(a.text,a.headers,a.status_code))


if __name__=='__main__':
  url = 'http://47.107.168.87:8080/futureloan/mvc/api/member/login'
  main = {'mobilephone': '1868873467', 'pwd': '123456'}
  G=w_1(url,main)
