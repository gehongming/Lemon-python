...
class RegisterCase(unittest.TestCase):
    excel = openexcel.DoExcel(contants.case_file, 'bindBankCard')
    cases = excel.read()
    @classmethod
    def setUpClass(cls):
        logger.info('准备测试前置')
    ...
    def test_register(self,url,data,method,expected,case_id,title,result,check_sql):
        ...
        card_id=create_data.create_card()
        logger.info('银行卡是：{}'.format(card_id))
        data=context.replace(data,'card_id',str(card_id))
        if case_id==1:
